...
def modal_callback(app, modal_id=None, modal_content_id=None, datatable_id=None, stored_data_id=None):
    @app.callback(
        Output(modal_id, 'is_open'),
        Output(modal_content_id, 'children'),
        Input(datatable_id, 'active_cell'),
        Input(datatable_id, 'page_current'),
        Input(stored_data_id, "data"),
        State(modal_id, "is_open"),
    )
    def toggle_modal(active_cell, page_current, data, is_open):
        ...
        if active_cell is None:
            ...
            incident = stored_data.iloc[row_on_current_page].to_dict()
            ...
            content = html.Div([
                dbc.Accordion(
                    [
                        ...
                        dbc.AccordionItem(
                            [html.P([html.Span("Data theft: ", style={"font-weight": "bold"}), f"{incident['data_theft']}"]),
                            html.P([html.Span("Disruption: ", style={"font-weight": "bold"}), f"{incident['disruption']}"]),
                            html.P([html.Span("Hijacking: ", style={"font-weight": "bold"}), f"{incident['hijacking']}"]),
                            html.P([html.Span("Physical effects (Temporal): ", style={"font-weight": "bold"}), f"{incident['physical_effects_temporal']}"]),
                            html.P([html.Span("Physical effects (Spatial): ", style={"font-weight": "bold"}), f"{incident['physical_effects_spatial']}"]),
                            html.P([html.Span("Unweighted Cyber Intensity: ", style={"font-weight": "bold"}), f"{incident['unweighted_cyber_intensity']}"]),
                            html.P([html.Span("Target/Effect multiplier: ", style={"font-weight": "bold"}), f"{incident['target_multiplier']}"]),
                            html.P([html.Span("Weighted Cyber Intensity: ", style={"font-weight": "bold"}), f"{incident['weighted_cyber_intensity']}"]),
                            html.P([html.Span("MITRE-Initial Access: ", style={"font-weight": "bold"}), f"{incident['MITRE_initial_access']}"]),
                            html.P([html.Span("MITRE-Impact: ", style={"font-weight": "bold"}), f"{incident['MITRE_impact']}"]),
                            html.P([html.Span("Common Vulnerability Scoring System-User Interaction: ", style={"font-weight": "bold"}), f"{incident['user_interaction']}"]),
                            html.P([html.Span("Zero day: ", style={"font-weight": "bold"}), f"{incident['zero_days']}"])],
                            title="Technical Categories"
                        ),
                        dbc.AccordionItem(
                            [html.P([html.Span("Cyber conflict issue: ", style={"font-weight": "bold"}), f"{incident['cyber_conflict_issue']}"]),
                            html.P([html.Span("Offline conflict issue: ", style={"font-weight": "bold"}), f"{incident['offline_conflict_issue']}"]),
                            html.P([html.Span("Offline conflict name: ", style={"font-weight": "bold"}), f"{incident['offline_conflict_issue_subcode']}"]),
                            html.P([html.Span("Online conflict intensity: ", style={"font-weight": "bold"}), f"{incident['offline_conflict_intensity']}"]),
                            html.P([html.Span("Offline conflict intensity score: ", style={"font-weight": "bold"}), f"{incident['offline_conflict_intensity_subcode']}"]),
                            html.P([html.Span("Casualties: ", style={"font-weight": "bold"}), f"{incident['casualties']}"]),
                            html.P([html.Span("Number of political responses: ", style={"font-weight": "bold"}), f"{incident['number_of_political_responses']}"])],
                            title="Political Categories"
                        ),
                        dbc.AccordionItem(
                            [html.P([html.Span("State Responsibility Indicator: ", style={"font-weight": "bold"}), f"{incident['state_responsibility_indicator']}"]),
                            html.P([html.Span("IL Breach Indicator: ", style={"font-weight": "bold"}), f"{incident['IL_breach_indicator']}"]),
                            html.P([html.Span("Evidence for Sanctions Indicator: ", style={"font-weight": "bold"}), f"{incident['evidence_for_sanctions_indicator']}"]),
                            html.P([html.Span("Number of legal responses: ", style={"font-weight": "bold"}), f"{incident['number_of_legal_responses']}"])],
